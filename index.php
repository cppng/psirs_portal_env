...
$fresh_rows_in_db_2 = $db2->get_entries_since($tin_table_in_db2, "create_date", $last_date_tin_db1 );
...
if(count($fresh_rows_in_db_1) > 0){
        ...
        $count = count($fresh_rows_in_db_1);
        $db2->log("Found $count fresh rows for db2");
        $db2->insert_fresh_rows($tin_table_in_db2, $fresh_rows_in_db_1);
}
else{
...
if(count($fresh_rows_in_db_2) > 0){
        ...
        $count = count($fresh_rows_in_db_2);
        $db1->log("Found $count fresh rows for db1");
